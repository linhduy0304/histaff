...
class ApprovePending extends Component {
	constructor(props) {
		super(props);
		this.state = {
    ...
    componentWillReceiveProps = (nextProps) => {
        if(nextProps.lot.approvePending && nextProps.lot.approvePending !== this.props.lot.approvePending) {
            this.setState({
                data: nextProps.lot.approvePending
            })
        }
    };

    confirm = (empId, fromDate, toDate, signId, groupId, status) => {
        const body = {
            ID_REGGROUP: groupId,
            ID_EMPLOYEE: empId,
            TO_DATE: toDate,
        ...
        this.props.approveLeave(body, status)
    }

	renderAction = (empId, fromDate, toDate, signId, groupId) =>{
        return (
			<View style={{flexDirection: 'row', justifyContent: 'space-between'}}>
				<Button 
					title = 'Duyệt'
					marginTop = {10}
					onPress = {() => this.confirm(empId, fromDate, toDate, signId, groupId, 2)}
				...
				<Button
					...
					marginTop = {10}
					width={(screen.width-100)/2}
					onPress = {() => this.confirm(empId, fromDate, toDate, signId, groupId, 3)}
	...
	renderItem(data) {
		...
		<View style={{padding: 15,margin: 15,marginBottom: 0, backgroundColor: '#fff', borderRadius: 4}}>
			...
			{this.renderAction(data.ID_EMPLOYEE, data.FROM_DATE, data.TO_DATE, data.ID_SIGN, data.ID_REGGROUP)}
		</View>
		)
	}
    ...
    submit = () => {
        ...
        const body = {
            approveId: this.props.profile.user.EMPLOYEE_ID, // this.props.profile.user.EMPLOYEE_ID
            todate: time_end,
            fromdate: time_start,
            status: 1, //dang kys chow
            signId: value
        }
        this.props.getApproveLeave(body, 'late_early')
    }
...
import { getApproveLeave, approveLeave} from '../../../actions/lot';
...
const mapDispatchToProps = (dispatch) => {
    return {
        ...
        approveLeave: (body, status) => dispatch(approveLeave(body, status)),
    }
}

export default connect(mapStateToProps, mapDispatchToProps)(ApprovePending);
