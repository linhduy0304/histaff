...
class Approved extends Component {
    ...
    componentWillReceiveProps = (nextProps) => {
        if(nextProps.lot.approved && nextProps.lot.approved !== this.props.lot.approved) {
            this.setState({
                data: nextProps.lot.approved
            })
        }
    };
	...
	renderItem(data) {
		...
		<View style={{padding: 15,margin: 15,marginBottom: 0, backgroundColor: '#fff', borderRadius: 4}}>
			...
			<Text><Text>Trạng thái: </Text>Đã duyệt</Text>
		</View>
		)
	}
